...
class Toggle extends Component{
    ...
    render()
    {
       ...
       { this.state.submit &&  <div> <b>Movie Name:  </b> {this.state.Movie} <br/>
         <button className="btn btn-success" onClick={this.handleAgeButton}>{this.state.showDate? 'Hide Date':'Show Release Date'}</button>
       { this.state.showDate && <div><label>The Release Date for {this.state.Movie} is : </label>{dayjs(this.state.ReleaseDate).format('DD/MMM') }
       </div> }
